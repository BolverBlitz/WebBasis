 ...
 * @returns 
 */
const WebtokensCreate = (user, token, browser, language) => {
  return new Promise((resolve, reject) => {
    pool.query(`INSERT INTO webtokens (username, token, browser, language) VALUES ($1, $2, $3, $4)`, [user, token, browser, language], (err, result) => {
      if (err) { reject(new SQLError(err)) }
      resolve(result)
    })
  })
